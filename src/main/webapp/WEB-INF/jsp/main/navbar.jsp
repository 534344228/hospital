...
<script type="text/javascript">
	...
	$(function() {
		$('#swidth').width($('#mwidth').innerWidth()).css('min-width',
				$('#mwidth').innerWidth())
		$('#se').select2({
			language : 'zh-CN',
			placeholder : "请输入关键字",
		...
		$("#se").on(
				"select2:select",
				function(e) {
					var url = "${path}/" + e.params.data.table_name + "/edit"
							+ replaceStr(e.params.data.table_name) + "?"
							+ e.params.data.table_name + "Id="
							+ e.params.data.id + "&pageFlag=true";
